...
class State:
    def __init__(self, id, costs, time = 0, robots = [1,0,0,0], resources=[0,0,0,0], prog_min = 0, prog_max = 1):
        self.id = id
        self.costs = costs
        self.time = time
        self.robots = robots
    ...
    def __str__(self):
        return "id {} time {} robots {} resources {} progress {},{}".format(self.id, self.time, self.robots, self.resources,self.prog_min,self.prog_max)

    def copy(self):
        return State(self.id, self.costs, self.time, self.robots.copy(), self.resources.copy())
    ...
    def advance(self):
        ...
        for i in range(len(next_step)):
            next_step[i].prog_min = self.prog_min + i*(self.prog_max-self.prog_min)/len(next_step)
            next_step[i].prog_max = self.prog_min + (i+1)*(self.prog_max-self.prog_min)/len(next_step)
        #next_step.reverse()
        return next_step
...
def part_a(input):
    ...
    for line in input_generator(input):
        ...
        ge_ore = int(words[27])
        ge_ob = int(words[30])
        costs = [[ore_ore,0,0,0],[clay_ore,0,0,0],[ob_ore,ob_clay,0,0],[ge_ore,0,ge_ob,0]]
        ...
        while state_queue and count < 10000000:
            ...
            if state.time == 24:
                ...
                if (count %100000 == 0 or improved):
                    print("{}: {} done with {} best {}".format(count,state,geodes,best))
            else:
