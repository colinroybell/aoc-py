...
def part2(stones):
    ...
    rpx, rpy, rpz, rvx, rvy, rvz, t0, t1, t2 = sym.symbols(
        "rpx, rpy, rpz, rvx, rvy, rvz, t0, t1, t2"
    )
    eqs = [
        sym.Eq(rpx + rvx * t0, p[0].x + t0 * v[0].x),
...
def part_a(input, test_min=200000000000000, test_max=400000000000000):
    ...
    for i, s1 in enumerate(stones):
        for j, s2 in enumerate(stones[i + 1 :]):
            ...
            print(int_point)
            if (
                test_min <= int_point.x <= test_max
                and test_min <= int_point.y <= test_max
            ):
                count += 1
    ...
    for target in range(len(stones)):
        ...
        if not failed and not first:
            ...
            for i, s1 in enumerate(stones):
                ...
                (ip2, t, u) = line_intersect(
                    Vec3d(0, 0, 0), ip, p1 - base, p1 - base + v1
                )
                print(ip2, t, u)
                int_u = int(u + 0.5)
